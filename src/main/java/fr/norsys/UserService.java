...
public class UserService {
    ...
    public List<User> getUsersByCountryName(List<User> users, String countryName) {
        return users.stream()
                .filter(user -> countryName.equals(user.getAddress().getCountry().getName()))
                .collect(Collectors.toList());
    }

    public List<String> getCountriesNameFromUsers(List<User> users) {
        return users.stream().map(user -> user.getAddress().getCountry().getName())
                .collect(Collectors.toList());
    }
    ...
    public List<Country> getCountriesFromUsersDistinct(List<User> users) {
        return users.stream().map(user -> user.getAddress().getCountry())
                .distinct()
                .collect(Collectors.toList());
    }
    ...
    public List<String> getCountriesNameFromUsersCustomSorted(List<User> users) {
        ...
        return users.stream().map(user -> user.getAddress().getCountry().getName())
                .sorted((country1, country2) -> {
                    if ("Morocco".equals(country1)) {
                        return -1;
                    }
                    if ("Morocco".equals(country2)) {
                        return 1;
                    }
    ...
    public String getUserEmailCombined(List<User> users) {
        return users.stream()
                ...
                .reduce((emailsList, email) -> emailsList + "," + email)
                .orElse("");
    }
    ...
    public Integer getSumOfUsersLoginHours(List<User> users) {
       /* return  (int)users.stream()
                .collect(Collectors.summarizingInt(User::getLoginHours))
                .getSum();*/

        return users.stream()
                .mapToInt(User::getLoginHours)
                .sum();
    ...
    public Map<String, List<User>> getGroupedUserByCountryNameGrouping(List<User> users) {
        return users.stream()
                .collect(Collectors.groupingBy(user -> user.getAddress().getCountry().getName()
                ));
    }

    public String getUserEmailCombinedJoining(List<User> users) {
        return users.stream()
                .map(user -> user.getEmail().replaceAll("@.*|-.*",""))
                .collect(Collectors.joining(","));
    }

    public Map<Boolean, List<User>> getPartitionedUserByAge(List<User> users, int age) {
        return users.stream().collect(Collectors.partitioningBy(u -> u.getAge() >= age));
    }

    public Map<String, Long> getGroupedUserByCountryNameNumber(List<User> users) {
        return users.stream()
                .collect(Collectors.groupingBy(user -> user.getAddress().getCountry().getName(), Collectors.counting())

                );
    }

    public Map<Boolean, Map<String, Long>> getPartitionedUserByAgeGroupedByCountry(List<User> users, int age) {
        return users.stream().collect(Collectors.partitioningBy(user -> user.getAge() >= age,
    ...
    public Map<String, Integer> getCountriesWithSumOfLoginHours(List<User> users) {
        return users.stream()
                .collect(Collectors.groupingBy(user -> user.getAddress().getCountry().getName(),
                        Collectors.reducing(0, User::getLoginHours, Integer::sum)
                ));
    }
